...
if __name__ == '__main__':
    ...
    while True:
        ...
        if bank_choice == 4:
            break
        #메뉴의 이외의 번호를 입력 시 밑으로 내려가지 못하게 막아준다.
        ...
        while True:
            ...
            if menu_choice == 1:
                ...
                while True:
                    ...
                    # None은 false값이지만, 가독성, 직관성을 높이기 위해 is 연산자로 검사한다.
                ...
                # 계좌가 개설된다.
                # bank_choice는 어떤 은행에서 개설했는지를 담아준다
                # open_account()는 회원의 정보를 **kwargs로 받기때문에 모두풀어서 전달해준다.
            ...
            # 출금
                ...
                if user is not None:
                    if user['password'] == input(password_message):
                        ...
                        # 로그인한 회원의 은행이 국민은행 이라면 출금수수료가 50%기 때문에
                        # 잔액검사시 1.5를 곱해준다.
...
# user = Bank.check_account_number(account_number)
# 이과정을 거치면 check_account_number과정을 거치면서 user에 주소값을 담는다?


# 강사님 - 주소값 불러오고 주소값이랑 비교
#  현이 - 값을 먼저 받고 login static method 만들어서 값이랑 입력된 값이랑 맞는지 비교

# if user is not None
# user를 위에서 담았을때 None이 된다는건 값이 다르다? 없다? 는거니까 없지 않아야 합니다라는
# 조건을 담는건가요??
